...
class UpdateProductRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'product.content' => 'required|string|min:10|max:1000', // Mô tả chi tiết phải có ít nhất 10 ký tự và không vượt quá 1000 ký tự

            // Ảnh đại diện và album
            'img' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048', // Ảnh đại diện có thể để trống, nếu có phải hợp lệ
            'array_img' => 'nullable|array|min:1', // Mảng ảnh có thể để trống, nếu có phải có ít nhất 1 ảnh
            'array_img.*' => 'image|mimes:jpeg,png,jpg,gif|max:2048', // Mỗi ảnh trong mảng phải là ảnh hợp lệ

            // Thông tin size
            'product_sizes' => 'nullable|array|min:1', // Mảng thông tin size có thể để trống, nếu có phải có ít nhất 1 size
            'product_sizes.*.variant' => 'required_with:product_sizes.*|string|max:100', // Tên size (bắt buộc nếu có size)
            'product_sizes.*.img' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048', // Ảnh của size (có thể để trống, nếu có phải hợp lệ)
            'product_sizes.*.price' => 'required_with:product_sizes.*|numeric|min:0|max:100000000', // Giá của size
            'product_sizes.*.quantity' => 'required_with:product_sizes.*|integer|min:0|max:100000', // Số lượng của size
            'product_sizes.*.status' => 'nullable|boolean', // Trạng thái của size (có thể để trống)
        ];
    }
