...
<form action="{{ route('admin.products.store') }}" method="POST" id="createproduct-form" autocomplete="off" class="needs-validation" novalidate enctype="multipart/form-data">
    @csrf
    <div class="row">
        <!-- Left column -->
        <div class="col-lg-8">
                ...
                <div class="card-body">
                    <div class="row mb-3">
                        <div class="col-lg-8">
                            ...
                            <input type="text" value="{{ old('product.name') }}" name="product[name]" class="form-control" placeholder="Tên sản phẩm" id="product_name">
                        ...
                        <div class="col-lg-4">
                            ...
                            <input type="text" name="product[price]" value="{{ old('product.price') }}" class="form-control" placeholder="Giá sản phẩm" id="product_price">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="product[description]" id="product_description" class="form-control" rows="1" placeholder="Mô tả ngắn">{{ old('product.description') }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="product[content]" id="product_content" class="form-control" rows="7" placeholder="Mô tả chi tiết sản phẩm">{{ old('product.content') }}</textarea>
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    ...
                    <label class="switch">
                        <input {{ old('product.status') == 1 ? 'checked' : '' }} name="product[status]" value="1" type="checkbox">
            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body text-center">
                    <input type="file" name="img" id="uploadImage" accept="image/*" onchange="previewImage(event)" class="form-control mb-3">
                    <div id="imagePreview" class="border rounded p-2" style="display:none;">
                        <img id="preview" class="img-fluid rounded" alt="Preview" style="width: 100%; object-fit: cover;">
                        ...
                        <tbody id="image-table-body">
                            <tr id="row_0">
                                <td class="d-flex align-items-center">
                                    <img id="preview_0" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS0Wr3oWsq6KobkPqznhl09Wum9ujEihaUT4Q&s" alt="Hình ảnh sản phẩm" class="me-3" style="width: 150px">
                                    <input type="file" id="img_0" name="array_img[0]" class="form-control" onchange="previewImages(this, 0)">
                                </td>
                                <td>
                                    <i class="mdi mdi-delete text-danger fs-18" style="cursor: pointer" onclick="removeRow(0)"></i>
                                </td>
                                <!-- Hiển thị lỗi cho hình ảnh đầu tiên -->
                                <td>
                                    @if ($errors->has('array_img.0'))
                                    <small class="text-danger">{{ $errors->first('array_img.0') }}</small>
                                    @endif
                                </td>
                            </tr>
                        </tbody>
            ...
            <div class="card-body">
                <table id="sizeTable" class="table table-bordered table-striped text-center">
                    ...
                    <tbody>
                        ...
                        <tr id="size_row_0">
                            <td>
                                <input type="text" class="form-control" name="product_sizes[0][variant]" placeholder="Nhập tên size">
                                @if ($errors->has('product_sizes.0.variant'))
                                <small class="text-danger">{{ $errors->first('product_sizes.0.variant') }}</small>
                                @endif
                            </td>
                            <td class="d-flex align-items-center">
                                <img id="size_preview_0" src="https://via.placeholder.com/150" alt="Hình ảnh sản phẩm" class="me-3" style="width: 150px;">
                                <input type="file" class="form-control" name="product_sizes[0][img]" onchange="previewSizeImage(this, 'size_preview_0')">
                                @if ($errors->has('product_sizes.0.img'))
                                <small class="text-danger">{{ $errors->first('product_sizes.0.img') }}</small>
                                @endif
                            </td>
                            <td>
                                <input type="number" class="form-control" name="product_sizes[0][price]" placeholder="Nhập giá">
                                @if ($errors->has('product_sizes.0.price'))
                                <small class="text-danger">{{ $errors->first('product_sizes.0.price') }}</small>
                                @endif
                            </td>
                            <td>
                                <input type="number" class="form-control" name="product_sizes[0][quantity]" placeholder="Nhập số lượng">
                                @if ($errors->has('product_sizes.0.quantity'))
                                <small class="text-danger">{{ $errors->first('product_sizes.0.quantity') }}</small>
                                @endif
                            </td>
                            <td>
        ...
        newRow.innerHTML = `
                ...
                <td>
                    <i class="mdi mdi-delete text-danger fs-18" style="cursor: pointer" onclick="removeRow(${rowCount})"></i>
                                    ...
                                    @if ($errors->has('array_img.${rowCount}'))
                                        <small class="text-danger">{{ $errors->first('array_img.${rowCount}') }}</small>
                                    @endif
                                </td>`;
